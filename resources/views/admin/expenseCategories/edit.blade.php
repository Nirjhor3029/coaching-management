    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.expense-categories.update", [$expenseCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $expenseCategory->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <label for="type">{{ trans('cruds.expenseCategory.fields.type') }}</label>
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $expenseCategory->type) }}">
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
