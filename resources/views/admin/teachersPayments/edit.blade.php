...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teachers-payments.update", [$teachersPayment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('payment_details') ? 'is-invalid' : '' }}" name="payment_details" id="payment_details">{{ old('payment_details', $teachersPayment->payment_details) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('month') ? 'is-invalid' : '' }}" type="number" name="month" id="month" value="{{ old('month', $teachersPayment->month) }}" step="1">
                @if($errors->has('month'))
                    <div class="invalid-feedback">
                        {{ $errors->first('month') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number" name="year" id="year" value="{{ old('year', $teachersPayment->year) }}" step="1">
                @if($errors->has('year'))
                    <div class="invalid-feedback">
                        {{ $errors->first('year') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_status') ? 'is-invalid' : '' }}" name="payment_status" id="payment_status">
                    ...
                    @foreach(App\Models\TeachersPayment::PAYMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_status', $teachersPayment->payment_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
