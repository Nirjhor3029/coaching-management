...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.studentBasicInfo.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-basic-infos.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('roll') ? 'is-invalid' : '' }}" type="number" name="roll" id="roll" value="{{ old('roll', '') }}" step="1">
                @if($errors->has('roll'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('class') ? 'is-invalid' : '' }}" name="class_id" id="class_id">
                    @foreach($classes as $id => $entry)
                        <option value="{{ $id }}" {{ old('class_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('class'))
                    <div class="invalid-feedback">
